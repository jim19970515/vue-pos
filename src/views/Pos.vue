  ...
  >
    ...
    >
      <nav class="flex items-center gap-2 sm:gap-4">
        <img
          src="/src/assets/posLogo.png"
          class="object-cover h-8 sm:h-20"
          alt=""
        />
        <h1>心心Pos</h1>
      </nav>
      <div class="flex items-center gap-6">
        <h2
          class="text-lg sm:text-2xl text-white font-medium text-center sm:block hidden"
        >
          {{ newDate }}
        </h2>
    ...
    <main class="sm:grid sm:grid-cols-4 h-full dark:bg-gray-500">
      <section
        class="sm:col-span-3 flex flex-col justify-between h-full relative"
      >
        <RouterViewTransition :route-key="route.path" />
        <pagination />
      </section>
      <section
        class="sm:col-span-1 bg-gray-400 p-4 hidden h-full sm:block dark:bg-gray-600 relative"
      >
        <loading />
        <div class="flex flex-col gap-4 h-full">
          ...
          <productTable />
          <p
            v-if="customerStore.is_paid"
            class="dark:text-bg-gray-900 dark:bg-gray-400 py-2 text-center rounded-md"
          >
            已付款
          </p>
          <p v-else class="dark:text-white py-2 text-center rounded-md">
            未付款
          </p>
          <button
